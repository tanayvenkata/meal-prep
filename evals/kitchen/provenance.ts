...
export function evaluationProvenance(model: string) {
  ...
  const paths = [...new Set(git(["ls-files", "-z", "--cached", "--others", "--exclude-standard", "--", "src", "evals", "supabase/migrations", "scripts/eval-kitchen.mjs", "package.json", "pnpm-lock.yaml", "pnpm-workspace.yaml", ".npmrc"]).split("\0").filter(Boolean))].sort();
