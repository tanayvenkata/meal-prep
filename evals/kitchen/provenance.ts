...
export function evaluationProvenance(model: string) {
  ...
  const paths = [...new Set(git(["ls-files", "-z", "--cached", "--others", "--exclude-standard", "--", "src", "evals", "scripts/eval-kitchen.mjs", "package.json", "package-lock.json"]).split("\0").filter(Boolean))].sort();
